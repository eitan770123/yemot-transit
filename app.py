...
def clean_text(text: str) -> str:
    ...
    if not text:
        return ""
    # הסרה מוחלטת של כל סימני הפיסוק (פסיקים, נקודות, מקפים, גרשיים וכו')
    # נקודה (.) משמשת בימות המשיח כמפריד קבצי שמע ולכן אסור להשתמש בה בתוך הטקסט
    text = re.sub(r'[.,\-\"\'\&|:;\u200e\u200f]', " ", text)
    # צמצום רווחים כפולים
...
def get_walking_instructions(line: str) -> tuple[str, str]:
    ...
    if line in {"74", "174", "274", "156"}:
        ...
        station = "מגדל שלום ברחוב אחד העם"
        walk = (
            "הוראות הליכה: רד בתחנת מגדל שלום. "
            "לך מזרחה ברחוב אחד העם כ-2 דקות, פנה ימינה לנחלת בנימין, "
            "ומפגש הרחובות רוטשילד ונחלת בנימין יהיה מיד לפניך."
        )
    else:
...
def handle_ivr_request():
    ...
    if not select or select == "9":
        session.clear()
        menu_msg = get_main_menu_response(phone, saved_route, session)
        return Response(make_ivr_response(menu_msg, phone, "select", 1, 1), mimetype="text/plain")
    ...
    if select == "0" and saved_route:
        ...
        if not saved_arrival:
            try:
                ...
                return Response(make_ivr_response("שגיאה בקבלת נתונים ממשרד התחבורה לחזרה לתפריט הקש 9", phone, "select", 1, 1), mimetype="text/plain")
                
            if not data:
                return Response(make_ivr_response("לא נמצאו קווים פעילים בתחנה זו כעת לחזרה לתפריט הקש 9", phone, "select", 1, 1), mimetype="text/plain")
        ...
        if saved_arrival:
            ...
            return Response(make_ivr_response(msg, phone, "select", 1, 1), mimetype="text/plain")
        else:
    ...
    if select == "4":
        session.clear()
        return Response(make_ivr_response("נא הקש את מזהה התחנה בן חמש הספרות", phone, "select", 5, 5, 10), mimetype="text/plain")
    ...
    if len(select) == 5 and select.isdigit():
        ...
        try:
            ...
            return Response(make_ivr_response("אירעה שגיאה בחיפוש התחנה אנא נסה שוב לחזרה לתפריט הקש 9", phone, "select", 1, 1), mimetype="text/plain")
            
        if not data:
            return Response(make_ivr_response("לא נמצאו קווים פעילים בתחנה זו לחזרה לתפריט הקש 9", phone, "select", 1, 1), mimetype="text/plain")
        ...
        if not ta_arrivals:
            return Response(make_ivr_response("לא נמצאו קווים ישירים לתל אביב בתחנה זו לחזרה לתפריט הקש 9", phone, "select", 1, 1), mimetype="text/plain")
        ...
        return Response(make_ivr_response(raw_msg, phone, "select", 1, 1), mimetype="text/plain")
    ...
    if select == "8":
        ...
        return Response(make_ivr_response(f"{walk_instructions} לחזרה לתפריט הראשי הקש 9", phone, "select", 1, 1), mimetype="text/plain")
    ...
    if select in {"1", "2", "3"}:
        ...
        if search_stop:
            try:
                ...
                pass
            session.clear()
            return Response(make_ivr_response(get_main_menu_response(phone, saved_route, session), phone, "select", 1, 1), mimetype="text/plain")
